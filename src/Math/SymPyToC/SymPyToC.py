...
# This is the same as T.shape if all indices are free. But of one index is
# fixed (like the 1 in T[a,1]) it returns a smaller shape for this index.
def get_pseudoshape_from_indexranges(Tranges):
    shap = tuple( [(rn[1]-rn[0]+1) for rn in Tranges] )
    return shap

# check if tensor has shape, that happens only if all indices are free ones
def has_shape(T):
    try:
        Tshape = T.shape
    except:
...
def get_tuple_with_all_indexvals(T):
    ...
    #print(Tranges)
    shap = get_pseudoshape_from_indexranges(Tranges)
    #print('shap =', shap)
...
def write_Eqs(filename, allEqs, AUTOVARS, FinalSimplify=True):
    global SymPyToC_pool
    if FinalSimplify == True:
        ...
        # undo sympy.factor_terms or sympy.gcd_terms. Why is that??? Weird!!!
...
###############################################################
# M.det() and especially M.inv() are very slow in 4d or more.
# So we offer special functions for them.
###############################################################

# compute det of rank 2 tensor Tij in 4d
def matrixdet4(Tij):
    T = matrix_from_tensor(Tij)
    # pre-calculated M.det() assmming indices run from 0 to 3
    Mdet = ( T[0,0]*T[1,1]*T[2,2]*T[3,3] - T[0,0]*T[1,1]*T[2,3]*T[3,2] -
    T[0,0]*T[1,2]*T[2,1]*T[3,3] + T[0,0]*T[1,2]*T[2,3]*T[3,1] +
    T[0,0]*T[1,3]*T[2,1]*T[3,2] - T[0,0]*T[1,3]*T[2,2]*T[3,1] -
    T[0,1]*T[1,0]*T[2,2]*T[3,3] + T[0,1]*T[1,0]*T[2,3]*T[3,2] +
    T[0,1]*T[1,2]*T[2,0]*T[3,3] - T[0,1]*T[1,2]*T[2,3]*T[3,0] -
    T[0,1]*T[1,3]*T[2,0]*T[3,2] + T[0,1]*T[1,3]*T[2,2]*T[3,0] +
    T[0,2]*T[1,0]*T[2,1]*T[3,3] - T[0,2]*T[1,0]*T[2,3]*T[3,1] -
    T[0,2]*T[1,1]*T[2,0]*T[3,3] + T[0,2]*T[1,1]*T[2,3]*T[3,0] +
    T[0,2]*T[1,3]*T[2,0]*T[3,1] - T[0,2]*T[1,3]*T[2,1]*T[3,0] -
    T[0,3]*T[1,0]*T[2,1]*T[3,2] + T[0,3]*T[1,0]*T[2,2]*T[3,1] +
    T[0,3]*T[1,1]*T[2,0]*T[3,2] - T[0,3]*T[1,1]*T[2,2]*T[3,0] -
    T[0,3]*T[1,2]*T[2,0]*T[3,1] + T[0,3]*T[1,2]*T[2,1]*T[3,0] )
    return Mdet #FIXME: this is not an Indexed obj

# compute inverse(Tij)*det(Tij) for a rank 2 tensor in 4d
def matrixinvdet4(Tij):
    T = matrix_from_tensor(Tij)
    # pre-calculated M.inv()*M.det() assmming indices run from 0 to 3
    Minvdet = sympy.Matrix([[T[1,1]*T[2,2]*T[3,3] - T[1,1]*T[2,3]*T[3,2] -
    T[1,2]*T[2,1]*T[3,3] + T[1,2]*T[2,3]*T[3,1] + T[1,3]*T[2,1]*T[3,2] -
    T[1,3]*T[2,2]*T[3,1],
    -T[0,1]*T[2,2]*T[3,3] + T[0,1]*T[2,3]*T[3,2] +
    T[0,2]*T[2,1]*T[3,3] - T[0,2]*T[2,3]*T[3,1] - T[0,3]*T[2,1]*T[3,2] +
    T[0,3]*T[2,2]*T[3,1],
    T[0,1]*T[1,2]*T[3,3] - T[0,1]*T[1,3]*T[3,2] -
    T[0,2]*T[1,1]*T[3,3] + T[0,2]*T[1,3]*T[3,1] + T[0,3]*T[1,1]*T[3,2] -
    T[0,3]*T[1,2]*T[3,1],
    -T[0,1]*T[1,2]*T[2,3] + T[0,1]*T[1,3]*T[2,2] +
    T[0,2]*T[1,1]*T[2,3] - T[0,2]*T[1,3]*T[2,1] - T[0,3]*T[1,1]*T[2,2] +
    T[0,3]*T[1,2]*T[2,1]],
    [-T[1,0]*T[2,2]*T[3,3] + T[1,0]*T[2,3]*T[3,2] +
    T[1,2]*T[2,0]*T[3,3] - T[1,2]*T[2,3]*T[3,0] - T[1,3]*T[2,0]*T[3,2] +
    T[1,3]*T[2,2]*T[3,0],
    T[0,0]*T[2,2]*T[3,3] - T[0,0]*T[2,3]*T[3,2] -
    T[0,2]*T[2,0]*T[3,3] + T[0,2]*T[2,3]*T[3,0] + T[0,3]*T[2,0]*T[3,2] -
    T[0,3]*T[2,2]*T[3,0],
    -T[0,0]*T[1,2]*T[3,3] + T[0,0]*T[1,3]*T[3,2] +
    T[0,2]*T[1,0]*T[3,3] - T[0,2]*T[1,3]*T[3,0] - T[0,3]*T[1,0]*T[3,2] +
    T[0,3]*T[1,2]*T[3,0],
    T[0,0]*T[1,2]*T[2,3] - T[0,0]*T[1,3]*T[2,2] -
    T[0,2]*T[1,0]*T[2,3] + T[0,2]*T[1,3]*T[2,0] + T[0,3]*T[1,0]*T[2,2] -
    T[0,3]*T[1,2]*T[2,0]],
    [T[1,0]*T[2,1]*T[3,3] - T[1,0]*T[2,3]*T[3,1] -
    T[1,1]*T[2,0]*T[3,3] + T[1,1]*T[2,3]*T[3,0] + T[1,3]*T[2,0]*T[3,1] -
    T[1,3]*T[2,1]*T[3,0],
    -T[0,0]*T[2,1]*T[3,3] + T[0,0]*T[2,3]*T[3,1] +
    T[0,1]*T[2,0]*T[3,3] - T[0,1]*T[2,3]*T[3,0] - T[0,3]*T[2,0]*T[3,1] +
    T[0,3]*T[2,1]*T[3,0],
    T[0,0]*T[1,1]*T[3,3] - T[0,0]*T[1,3]*T[3,1] -
    T[0,1]*T[1,0]*T[3,3] + T[0,1]*T[1,3]*T[3,0] + T[0,3]*T[1,0]*T[3,1] -
    T[0,3]*T[1,1]*T[3,0],
    -T[0,0]*T[1,1]*T[2,3] + T[0,0]*T[1,3]*T[2,1] +
    T[0,1]*T[1,0]*T[2,3] - T[0,1]*T[1,3]*T[2,0] - T[0,3]*T[1,0]*T[2,1] +
    T[0,3]*T[1,1]*T[2,0]],
    [-T[1,0]*T[2,1]*T[3,2] + T[1,0]*T[2,2]*T[3,1] +
    T[1,1]*T[2,0]*T[3,2] - T[1,1]*T[2,2]*T[3,0] - T[1,2]*T[2,0]*T[3,1] +
    T[1,2]*T[2,1]*T[3,0],
    T[0,0]*T[2,1]*T[3,2] - T[0,0]*T[2,2]*T[3,1] -
    T[0,1]*T[2,0]*T[3,2] + T[0,1]*T[2,2]*T[3,0] + T[0,2]*T[2,0]*T[3,1] -
    T[0,2]*T[2,1]*T[3,0],
    -T[0,0]*T[1,1]*T[3,2] + T[0,0]*T[1,2]*T[3,1] +
    T[0,1]*T[1,0]*T[3,2] - T[0,1]*T[1,2]*T[3,0] - T[0,2]*T[1,0]*T[3,1] +
    T[0,2]*T[1,1]*T[3,0],
    T[0,0]*T[1,1]*T[2,2] - T[0,0]*T[1,2]*T[2,1] -
    T[0,1]*T[1,0]*T[2,2] + T[0,1]*T[1,2]*T[2,0] + T[0,2]*T[1,0]*T[2,1] -
    T[0,2]*T[1,1]*T[2,0]]])
    return matrix_to_tensor(Minvdet, Tij.indices)

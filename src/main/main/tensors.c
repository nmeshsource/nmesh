...
void tensorindexlist(const char *tensind, int *nilist, char **ilist, int *sym)
{
  ...
  if(strcmp(tensorindices, "ijkl+jikl") == 0 ||
     strcmp(tensorindices, "(ij)kl") == 0) {
    for(i = 0; i < 3; i++)
    for(j = i; j < 3; j++)
    for(k = 0; k < 3; k++)
  ...
  if(strcmp(tensorindices, "ijkl+ikjl") == 0 ||
     strcmp(tensorindices, "i(jk)l") == 0) {
    for(i = 0; i < 3; i++)
    for(j = 0; j < 3; j++)
    for(k = j; k < 3; k++)
    for(l = 0; l < 3; l++)
  ...
  if(strcmp(tensorindices, "ia") == 0) {
    ...
    for(j = 0; j <= 3; j++) {
      ...
      if(j > 0) sym[3*n+j-1] *= -1;
      ilist[n] = calloc(ilistSTRLEN, sizeof(char));
      sprintf(ilist[n++], "%s%s", coord[i], coord4[j]);
    }
  }
  ...
  if(strcmp(tensorindices, "ijab+ijba") == 0 ||
     strcmp(tensorindices, "ij(ab)") == 0) {
    for(i = 0; i < 3; i++)
    for(j = 0; j < 3; j++)
    for(k = 0; k <= 3; k++)
